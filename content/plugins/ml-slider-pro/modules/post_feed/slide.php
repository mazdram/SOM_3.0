...
class MetaPostFeedSlide extends MetaSlide {
    ...
    protected function get_admin_slide() {
        ...
        $row .= "    <td class='col-2'>";
        $row .= "        <ul class='tabs'>";
        $row .= "            <li rel='tab-1' class='selected codemirror custom_template' data-editor='editor{$this->slide->ID}'>" . __( "Caption Template", "metasliderpro" ) . "</li>";
        $row .= "            <li rel='tab-2'>" . __( "Post Types", "metasliderpro" ) . "</li>";
        $row .= "            <li rel='tab-3'>" . __( "Taxonomies", "metasliderpro" ) . "</li>";
        $row .= "            <li rel='tab-4'>" . __( "Display Settings", "metasliderpro" ) . "</li>";
        $row .= "        </ul>";
        $row .= "        <div class='tabs-content'>";
        $row .= "            <div class='tab tab-1'>";
        $row .= "                <div style='position: relative;'>";
        $row .= "                    {$this->get_template_tags()}";
        $row .= "                </div>";
        $row .= "                <div class='row last'>";
        $row .= "                    <textarea class='wysiwyg' id='editor{$this->slide->ID}' name='attachment[{$this->slide->ID}][settings][custom_template]'>{$custom_template}</textarea>";
        $row .= "                </div>";
        $row .= "            </div>";
        $row .= "            <div class='tab tab-2' style='display: none;'><p>" . __( "Select the Post types to include in the feed. Posts must have a Featured Image to appear in the feed.", "metasliderpro" ) . "</p>{$this->get_post_type_options()}</div>";
        $row .= "            <div class='tab tab-3' style='display: none;'><p>" . __( "Posts must be tagged to at least one of the selected categories to display in the feed.", "metasliderpro" ) . "</p>{$this->get_tag_options()}</div>";
        ...
        $row .= "                </div>";
        $row .= "                <div class='row'>";
        $row .= "                    <label>" . __( "Preserve New Lines", "metasliderpro" ) . "</label>";
        $row .= "                    <input type='checkbox' name='attachment[{$this->slide->ID}][settings][nl2br]' {$nl2br_checked}/>";
        $row .= "                </div>";
        $row .= "            </div>";
        $row .= "        </div>";
    ...
    private function get_flex_slider_markup( $slide ) {
        ...
        if ( version_compare( METASLIDER_VERSION, 2.3, '>=' ) ) {
            ...
            $html = "<li class='{$slide['class']}' style='display: none; float: left;'{$thumb}>" . $html . "</li>";
        }
    ...
    private function get_responsive_slides_markup( $slide ) {
        ...
        if ( strlen( $slide['caption'] ) ) {
            if ( $this->slide_settings['caption'] != 'custom_template_overlay' ) {
                $html .= "<div class='caption-wrap'>";
                $html .= "<div class='caption'>" . $slide['caption'] . "</div>";
                $html .= "</div>";
            } else {
                $html .= "<div class='msHtmlOverlay'>" . $slide['caption'] . "</div>";
            }
        }
